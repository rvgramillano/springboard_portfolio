...
# collect electricty data for Seattle
REGION_CODE = 'SCL'
...
def fix_date(df):
	...
	for j,date in enumerate(df['date']):
		...
		if j == len(df['date'])-1:
			# if the last hour has more than one entry, perform same process of picking min date and dropping the rest
			if len(hour_dict) >= 1:
				hour_dict[date] = j
				...
				continue
			# otherwise, just append the last date
			else:
...
weather_df['hourlyvisibility'] = weather_df['hourlyvisibility'].apply(lambda x: float(x) if str(x)[-1] != 'V' and str(x)[-1] != 's' else float(str(x)[:-1]))
...
weather_df['hourlyaltimetersetting'] = weather_df['hourlyaltimetersetting'].apply(lambda x: float(x) if str(x)[-1] != 's' else float(str(x)[:-1]))
...
plt.tight_layout()
ax.legend()
plt.savefig(WORKING_DIR + 'plots/distributions_%s/electricity_data.png' % CITY, dpi=350)
plt.close()
...
for col in electricity_df.columns:
	...
	plt.tight_layout()
	plt.savefig(WORKING_DIR + 'plots/distributions_%s/%s_dist.png' % (CITY, col), dpi=350)
	plt.close()
...
# save as pickle file
final_df.to_pickle(WORKING_DIR + '%s_df.pkl' % CITY)
